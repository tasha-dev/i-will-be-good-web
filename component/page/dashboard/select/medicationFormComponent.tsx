...
export default function MedicationFormComponent():ReactNode {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmitHandler)} className={'grid lg:grid-cols-5 grid-cols-1 gap-[10px] mb-[20px]'}>
            ...
            <TimePickerComponent registerName={'time'} register={register} errorText={errors.time?.message} onChange={(event) => console.log(event)} className={'lg:col-span-3 col-span-1'} />
            <div>
                <SubmitBtnComponent className={'lg:w-auto w-full'} isValidating={isValidating} />
            </div>
        </form>
    );
}